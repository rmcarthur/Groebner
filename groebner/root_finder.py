...
def roots(polys):
    ...
    rand_poly = _random_poly(poly_type)

    # Get multiplication matrix
    m_f = multMatrix(rand_poly, GB, vectorSpaceBasis(GB))
...
def _random_poly(_type):
    rand_poly_coeff = np.array([[[0, np.random.randint(1000)],
                                [np.random.randint(1000), 0]],

                                [[np.random.randint(1000), 0],
                                [0,0]]])

    if _type == 'MultiCheb':
        return MultiCheb(rand_poly_coeff)
    else:
        return MultiPower(rand_poly_coeff)
